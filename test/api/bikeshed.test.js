...
var {request, fixtures, buildHeaders, models} = require('./helper'),
  jpgPath = path.join(fixtures, 'puppy_01.jpg'),
  svgPath = path.join(fixtures, 'invalid.svg'),
  {Bikeshed, Bike, User, Vote} = models,
  {expect} = require('chai')
  ...
  describe('POST /api/bikesheds/:bikeshed/bikes', function () {
    ...
    it('allows multiple users to vote at the same time', function* () {
      ...
      yield (yield _.times(5, () => User.create()))
        .map(user => buildHeaders({user: {id: user.id}}))
